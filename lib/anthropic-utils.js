...
function getSeasonalContext() {
  ...
  if (month >= 11 || month === 1)  return 'Q4/holiday peak: high demand for expedited beverage shipping, inventory build-up before year-end, gift spirits and wine surge.';
  if (month >= 8 && month <= 10)   return 'Fall: back-to-shelf restocking, craft beverage season, pre-holiday inventory planning for spirits and wine.';
  if (month >= 5 && month <= 7)    return 'Summer: RTD and cold beverage demand surges, festival and event logistics peak, energy drink sales spike.';
  return 'Q1: post-holiday inventory rebalancing, new product launches, beverage brands planning distribution strategy for the year.';
}
...
Generate exactly 5 SEO-driven content ideas. Spread them across different content types based on gaps identified above. Return a valid JSON array where each object has exactly these fields:
{
