...
const TRENDS_BLOB_PATHNAME = 'trends/trends.json';
...
// ---------------------------------------------------------------------------
// TRENDS — read/write cached vertical trend data
// ---------------------------------------------------------------------------
async function readTrends() {
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    const { list } = require('@vercel/blob');
    try {
      const { blobs } = await list({ prefix: TRENDS_BLOB_PATHNAME });
...
async function writeTrends(data) {
  if (process.env.BLOB_READ_WRITE_TOKEN) {
    const { put } = require('@vercel/blob');
    await put(TRENDS_BLOB_PATHNAME, JSON.stringify(data, null, 2), {
      access: 'public',
      contentType: 'application/json',
      addRandomSuffix: false,
      allowOverwrite: true,
...
module.exports = { readIdeas, writeIdeas, readTrends, writeTrends };
