...
Ideas are generated by combining **SEMrush keyword data** (beverage logistics, beer distribution, cold chain, etc.) with **Claude AI web search** for current industry news, then reviewed and approved through a branded dashboard.
...
|---|---|
| `SEMRUSH_API_KEY` | Your SEMrush API key — from [semrush.com/api-documentation](https://www.semrush.com/api-documentation/) |
...
├── lib/
│   ├── semrush.js          # SEMrush phrase_related keyword API calls
...
The generator queries the SEMrush `phrase_related` endpoint for five seed topics:
- `beverage logistics`
- `beer distribution`
...
Each call returns up to 20 related keywords sorted by search volume. Results are deduplicated and the top 20 by volume are passed to Claude for idea generation.

If the SEMrush API is unavailable (e.g., no key set), the generator falls back to a curated set of 20 seed keywords with estimated volumes so content generation still runs.
